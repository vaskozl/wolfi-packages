...
# Build the set of packages that are local dependencies of other packages.
# Only these need to upload artifacts so downstream jobs can install them.
# Packages with no local dependents (e.g. large final packages like seerr)
# skip artifact upload, avoiding GitLab's max_artifacts_size 413 errors.
while IFS='|' read -r base _ _ deps; do
    resolve_deps "$base" "$deps"
done <"$WORK/rows" | sort -u >"$WORK/is-dep"

is_local_dep() { grep -qxF "$1" "$WORK/is-dep"; }
...
YAML
        ...
        # Upload built packages as artifacts only for packages that are
        # local dependencies of other packages in this repo.  This lets
        # downstream jobs install them via --repository-append=$OUT_DIR.
        # Packages with no local dependents skip artifact upload to avoid
        # hitting GitLab's max_artifacts_size limit (seerr ~932 MB, etc.).
        if is_local_dep "$base"; then
            cat <<YAML
  artifacts:
    when: on_success
    expire_in: 1 day
